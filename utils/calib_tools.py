...
class GeneralCalibrationError():
  ...
  def get_calibration_error(self, probs, labels, bin_upper_bounds, norm,
    ...
    self.confidences = sums / counts  #Hanqi: get average of probs passed in separately for each bin, 
    self.accuracies = np.bincount(
        bin_indices, weights=labels, minlength=num_bins) / counts # This is just weighted average of the labels, which could be cor or could be isTrue.
    ...
    if norm == 'l1':
      ...
      return np.sum(weighted_calibration_error)
  ...
  def update_state(self, labels, probs):
    ...
    if not self.class_conditional:
      if self.max_prob: # Hanqi: ECE uses max_prob
        labels_matrix = labels_matrix[
            range(len(probs)), np.argmax(probs, axis=1)] # Hanqi: Takes the max probs for you, I guess when passing in binaries, nothing happens
    ...
    # Hanqi: ECE does not use the below
...
def signed_ece(labels, probs, num_bins=30):
  """Implements Signed Expected Calibration Error, ECE with no absolute value applied."""
  return gce(labels,
             probs,
             binning_scheme='even',
             max_prob=True,
